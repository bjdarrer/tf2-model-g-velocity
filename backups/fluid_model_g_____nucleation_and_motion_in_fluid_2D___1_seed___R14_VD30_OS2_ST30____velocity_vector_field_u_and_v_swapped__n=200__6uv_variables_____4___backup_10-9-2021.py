...
class FluidModelG(PDESolverDx):
    ...
    def __init__(self, concentration_G, concentration_X, concentration_Y, u, dx, dt=None, params=None, source_functions=None):
        ...
        if self.dims == 1:
            ...
            """
            #v = tf.Variable(1.)
            # BJD making 6 variable of u and v - 10.9.2021
            self.u_X = tf.Variable(u[0], 'float64')
            self.v_X = tf.Variable(u[0], 'float64')
            self.u_Y = tf.Variable(u[0], 'float64')
            self.v_Y = tf.Variable(u[0], 'float64')
            self.u_G = tf.Variable(u[0], 'float64')
            self.v_G = tf.Variable(u[0], 'float64')
            ...
            """
            def flow_integrator1(rho, u_X, v_X):
                """
                Flow is integrated with respect to the total log density (rho)
                """
                # Enter Fourier Domain
                f_rho = self.fft(tf.cast(rho, 'complex128'))
                waves_x = self.fft(tf.cast(u_X, 'complex128'))
                waves_y = self.fft(tf.cast(v_X, 'complex128'))
                ...
                # Exit Fourier Domain
                u_X = tf.cast(self.ifft(waves_x), 'float64')
                v_X = tf.cast(self.ifft(waves_y), 'float64')
                ...
                # Self-advect flow
                du = -u_X*u_dx - v_X*u_dy
                dv = -u_X*v_dx - v_X*v_dy
                ...
                return u_X, v_X, divergence

            def flow_integrator2(rho, u_Y, v_Y):
                """
                Flow is integrated with respect to the total log density (rho)
                """
                # Enter Fourier Domain
                f_rho = self.fft(tf.cast(rho, 'complex128'))
                waves_x = self.fft(tf.cast(u_Y, 'complex128'))
                waves_y = self.fft(tf.cast(v_Y, 'complex128'))
                ...
                # Exit Fourier Domain
                u_Y = tf.cast(self.ifft(waves_x), 'float64')
                v_Y = tf.cast(self.ifft(waves_y), 'float64')
                ...
                # Self-advect flow
                du = -u_Y*u_dx - v_Y*u_dy
                dv = -u_Y*v_dx - v_Y*v_dy
                ...
                return u_Y, v_Y, divergence

            def flow_integrator3(rho, u_G, v_G):
                """
                Flow is integrated with respect to the total log density (rho)
                """
                # Enter Fourier Domain
                f_rho = self.fft(tf.cast(rho, 'complex128'))
                waves_x = self.fft(tf.cast(u_G, 'complex128'))
                waves_y = self.fft(tf.cast(v_G, 'complex128'))
                ...
                # Exit Fourier Domain
                u_G = tf.cast(self.ifft(waves_x), 'float64')
                v_G = tf.cast(self.ifft(waves_y), 'float64')
                ...
                # Self-advect flow
                du = -u_G*u_dx - v_G*u_dy
                dv = -u_G*v_dx - v_G*v_dy
        ...
        self.flow_integrator1 = tf.function(flow_integrator1)
        self.flow_integrator2 = tf.function(flow_integrator2)
        self.flow_integrator3 = tf.function(flow_integrator3)
    ...
    def step(self):
        ...
        if self.dims == 2:
            ...
            self.u_X, self.v_X, divergence1 = self.flow_integrator1(rho1, self.u_X, self.v_X) #--- BJD added 9.6.2021
            self.u_Y, self.v_Y, divergence2 = self.flow_integrator2(rho2, self.u_Y, self.v_Y) #--- BJD added 9.6.2021
            self.u_G, self.v_G, divergence3 = self.flow_integrator3(rho3, self.u_G, self.v_G) #--- BJD added 9.6.2021
            ...
            # =======================BJD 18.11.2020================================================      
            np.savetxt("/home/brendan/software/tf2-model-g-velocity/arrays/quiver_array1/u_X.txt", self.u_X) # BJD 1.2.2021
            np.savetxt("/home/brendan/software/tf2-model-g-velocity/arrays/quiver_array1/v_X.txt", self.v_X) # BJD 1.2.2021

            np.savetxt("/home/brendan/software/tf2-model-g-velocity/arrays/quiver_array1/u_Y.txt", self.u_Y) # BJD 1.2.2021
            np.savetxt("/home/brendan/software/tf2-model-g-velocity/arrays/quiver_array1/v_Y.txt", self.v_Y) # BJD 1.2.2021

            np.savetxt("/home/brendan/software/tf2-model-g-velocity/arrays/quiver_array1/u_G.txt", self.u_G) # BJD 1.2.2021
            np.savetxt("/home/brendan/software/tf2-model-g-velocity/arrays/quiver_array1/v_G.txt", self.v_G) # BJD 1.2.2021
